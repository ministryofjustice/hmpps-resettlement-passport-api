...
class YamlResettlementAssessmentIntegrationTest : IntegrationTestBase() {
  ...
  fun `Post resettlement assessment submit - happy path`() {
    ...
    val expectedAssessments = listOf(
      ...
      ResettlementAssessmentEntity(id = 4, prisonerId = 1, pathway = Pathway.DRUGS_AND_ALCOHOL, statusChangedTo = Status.DONE, assessmentType = ResettlementAssessmentType.BCST2, assessment = ResettlementAssessmentQuestionAndAnswerList(assessment = mutableListOf()), creationDate = LocalDateTime.parse("2023-01-09T19:02:45"), createdBy = "A User", assessmentStatus = ResettlementAssessmentStatus.SUBMITTED, caseNoteText = "Case note related to Drugs and alcohol", createdByUserId = "USER_1", submissionDate = fakeNow),
      ...
      ResettlementAssessmentEntity(id = 7, prisonerId = 1, pathway = Pathway.HEALTH, statusChangedTo = Status.NOT_STARTED, assessmentType = ResettlementAssessmentType.BCST2, assessment = ResettlementAssessmentQuestionAndAnswerList(assessment = mutableListOf()), creationDate = LocalDateTime.parse("2023-01-09T19:02:45"), createdBy = "A User", assessmentStatus = ResettlementAssessmentStatus.SUBMITTED, caseNoteText = "Case note related to Health", createdByUserId = "USER_1", submissionDate = fakeNow),
    )
  ...
  fun `Post resettlement assessment submit - delius issue - sendCombinedCaseNotes`() {
    // PSFR-1297 for now we will ignore and log any issues posting case notes to Delius
    val nomsId = "ABC1234"
    val crn = "123"
    val assessmentType = "BCST2"
    val prisonId = "MDI"
