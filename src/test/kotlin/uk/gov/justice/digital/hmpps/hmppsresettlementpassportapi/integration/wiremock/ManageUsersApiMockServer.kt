...
class ManageUsersApiMockServer : WireMockServerBase() {
  fun stubGetManageUsersData(prisonId: String, status: Int) {
    ...
    stubFor(
      get("/prisonusers/find-by-caseload-and-role?activeCaseload=MDI&roleCode=ADD_SENSITIVE_CASE_NOTES&status=ACTIVE&page=0&size=500&sort=firstName").willReturn(
        if (status == 200) {
          aResponse()
  ...
  fun stubGetManageUsersDataEmptyList(status: Int) {
    ...
    stubFor(
      get("/prisonusers/find-by-caseload-and-role?activeCaseload=MDI1&roleCode=ADD_SENSITIVE_CASE_NOTES&status=ACTIVE&page=0&size=500&sort=firstName").willReturn(
        if (status == 200) {
          aResponse()
            .withHeader("Content-Type", "application/json")
            .withBody(
