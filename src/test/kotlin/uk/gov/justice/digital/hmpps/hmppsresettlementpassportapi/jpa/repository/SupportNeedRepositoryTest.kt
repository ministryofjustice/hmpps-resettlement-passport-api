...
class SupportNeedRepositoryTest : RepositoryTestBase() {
  ...
  fun `test legacy support needs`() {
    ...
    val expectedSupportNeeds = getLegacySupportNeeds()
  ...
  @Test
  fun `test findAllBySectionAndDeletedIsFalse`() {
    supportNeedRepository.save(SupportNeedEntity(id = 100002, pathway = Pathway.ACCOMMODATION, section = "Legacy support need", title = "A title", hidden = false, excludeFromCount = false, allowOtherDetail = false, createdDate = LocalDateTime.parse("2025-01-14T15:08:55.437998"), deleted = true, deletedDate = LocalDateTime.parse("2025-01-31T16:14:00")))

    val expectedSupportNeeds = getLegacySupportNeeds()
    assertThat(supportNeedRepository.findAllBySectionAndDeletedIsFalse("Legacy support need")).usingRecursiveComparison().ignoringFields("createdDate").isEqualTo(expectedSupportNeeds)
  }

  private fun getLegacySupportNeeds() = listOf(
