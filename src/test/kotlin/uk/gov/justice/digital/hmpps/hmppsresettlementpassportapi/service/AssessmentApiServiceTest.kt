...
class AssessmentApiServiceTest {
  private lateinit var assessmentApiService: AssessmentApiService
  ...
  @BeforeEach
  fun beforeEach() {
    assessmentApiService = AssessmentApiService(assessmentRepository, prisonerRepository, idTypeRepository)
  }
  ...
  fun `test deleteAssessment - sets deleted flag`() = runTest {
    ...
    assessmentApiService.deleteAssessment(assessmentEntity)
  ...
  fun `test createAssessment - creates assessment`() = runTest {
    ...
    val assessmentDTO = AssessmentDTO(fakeNow, isBankAccountRequired = true, isIdRequired = true, idDocuments = setOf("Driving licence", "Birth certificate"), nomsId = nomsId)
