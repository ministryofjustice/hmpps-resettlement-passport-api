...
class DrugsAndAlcoholResettlementAssessmentStrategyTest {
  ...
  private fun setUpMocks(nomsId: String, returnResettlementAssessmentEntity: Boolean, assessment: ResettlementAssessmentQuestionAndAnswerList = ResettlementAssessmentQuestionAndAnswerList(listOf())) {
    ...
    val pathwayEntity = PathwayEntity(1, "DRUGS_AND_ALCOHOL", true, testDate)
    ...
    Mockito.`when`(pathwayRepository.findById(Pathway.DRUGS_AND_ALCOHOL.id)).thenReturn(Optional.of(pathwayEntity))
